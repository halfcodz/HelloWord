...
void main() {
  test('마크다운 4칸 표: 앞 2칸만, 헤더행 제외', () {
    ...
    expect(map['be proud of'], '~을 자랑스러워하다');
    expect(map['give up'], '포기하다');
    expect(map['in front of'], '~앞에');
    // 헤더행(단어/숙어)은 단어로 잡히면 안 된다.
    expect(map.containsKey('단어'), isFalse);
    expect(map.containsKey('숙어'), isFalse);
    // 뜻에 발음/예문이 섞이면 안 된다.
    expect(map['discover']!.contains('디스커버'), isFalse);
    expect(map['discover']!.contains('park'), isFalse);
